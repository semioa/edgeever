...
实例管理员可以在 **个人中心** -> **账号管理** 中创建、停用成员账号或重置密码，实例默认不开放公开注册。每个成员拥有完全隔离的个人空间，包括笔记本、笔记、附件、回收站、导入导出和 MCP Token。已有单用户实例升级时会自动把全部原数据归入原管理员的默认空间，不改变原有数据 ID。Cloudflare 免费额度由整个实例共同使用，不会按账号分别增加。
