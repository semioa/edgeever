...
也许有人会问：为什么不整体迁移到更开放的 Obsidian？这其实我最先尝试的方案。但实测了一段时间发现，它对于随手记的场景还是太重了，尤其是移动端；且官方的多端同步方案是收费的，每月 5 美元，第三方同步方案又太折腾。因此干脆舍弃。

Memos 等轻量笔记产品更开放，但交互体验和经典印象笔记式三栏工作流仍有明显距离。
EdgeEver 想填补这个空白：保留熟悉的笔记体验，同时提供开放数据、REST API、MCP 和零成本自托管部署。

我目前研究的最佳实践是：用 EdgeEver 随手记录灵感和备忘，作为初始的原料库；需要结构化整理的时候，再基于 MCP 让 AI 整合到 Obsidian、Notion Database 和飞书多维表格里面。
